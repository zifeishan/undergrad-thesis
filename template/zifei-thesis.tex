% vim:ts=4:sw=4
%
% Copyright (c) 2008-2009 solvethis
% Copyright (c) 2010-2013 Casper Ti. Vector
% Public domain.
%
% 使用前请先仔细阅读 pkuthss 和 biblatex-caspervector 的文档，
% 特别是其中的 FAQ 部分和用红色强调的部分。
% 两者可在终端/命令提示符中用
%   texdoc pkuthss
%   texdoc biblatex-caspervector
% 调出。

% 在黑白打印时彩色链接可能变成浅灰色，此时可将“colorlinks”改为“nocolorlinks”。
% \documentclass[UTF8, colorlinks]{pkuthss}
\documentclass[UTF8, colorlinks, adobefonts]{pkuthss}
% \documentclass[UTF8, nocolorlinks, adobefonts]{pkuthss}

% 使用 biblatex 排版参考文献，并规定其格式。
% 默认按照引用顺序排序（“sorting = none”），详见 biblatex-caspervector 的文档
% （因为是默认设置所以其实不用写，不过出于完备性的考虑仍然在这里列出）。
% 若需要按照英文文献在前，中文文献在后排序，请设置“sorting = ecnty”；
% 若需要按照中文文献在前，英文文献在后排序，请设置“sorting = centy”。
\usepackage[backend = biber, style = caspervector, utf8, sorting = none]{biblatex}
% 使得打字机粗体可以被使用。
\usepackage{lmodern}
% 产生 originauth.tex 里的 \square。
\usepackage{amssymb}
% 提供 Verbatim 环境和 \VerbatimInput 命令。
\usepackage{fancyvrb}

\renewcommand{\cite}{\parencite}  % 临时，全加上方括号

\usepackage[ruled,lined]{algorithm2e}
\usepackage{tabularx}
\usepackage{graphicx}
\usepackage{stfloats}
\usepackage{multirow}
\usepackage{multicol}
\usepackage{array}
\usepackage{subfigure}
\usepackage{epstopdf}
\usepackage{url}
\usepackage{amssymb,amsmath}
\usepackage{flushend}
\usepackage{microtype} % Margin kerning / font expansion
\usepackage{mathptmx} % Narrow font, designed for multiple columns

% \newtheorem{algorithm}{Algorithm}
\newtheorem{definition}{定义}
\newtheorem{theorem}{定理}


% 使被强调的内容为红色。
\newcommand{\myemph}[1]{\emph{\textcolor{red}{#1}}}

% pkuthss 文档模版的版本。
% \newcommand{\docversion}{v1.4 rc3}
% 设定文档的基本信息。
\pkuthssinfo{
	% cthesisname = {通用竞技比赛的建模及选手排序算法的设计、应用与分析}, ethesisname = {GameRank: Ranking Players by Competitions between Roles in Sports Networks},
	cthesisname={本科生毕业论文}, ethesisname={Undergraduate Thesis},
	% ctitle = {北京大学论文文档模版\\pkuthss \docversion},
	% ctitle = {通用竞技比赛的建模及选手排名算法的\\设计、应用与分析},
	ctitle = {GameRank: 一种面向体育竞技比赛的选手排名算法设计与实现},
	% “\\”在设定 pdf 属性时会被自动过滤掉，于是得到的 pdf 属性中标题为
	%   The PKU dissertation document classpkuthss [版本号]
	% 此处指定其被替换为“: ”，以使之为
	%   The PKU dissertation document class: pkuthss [版本号]
	% etitle = {%
	% 	The PKU dissertation document class\texorpdfstring{\\}{: }%
	% 	pkuthss \docversion%
	% },
	etitle = {%
	  GameRank: Ranking Players by Competitions between Roles in Sports Networks
	},
	cauthor = {单子非},
	eauthor = {Zifei Shan},
	studentid = {00948181},
	date = {二〇一三年五月},
	school = {信息科学技术学院},
	cmajor = {计算机科学技术系}, emajor = {Computer Science},
	direction = {社会网络与数据挖掘},
	cmentor = {代亚非 教授}, ementor = {Prof.\ Yafei Dai},
	ckeywords = {棒球, 社会网络, 网络科学, 选手排名, 数据挖掘, 数据分析},
	ekeywords = {baseball, social networks, network science, ranking, data mining, data analysis},
}
% 导入参考文献数据库（注意不要省略“.bib”）。
\addbibresource{zifei-thesis.bib}

\begin{document}
	% 以下为正文之前的部分。
	\frontmatter

	% 自动生成标题页。
	\maketitle
	% 版权声明。
	% \include{chap/copyright}
	
	% 中英文摘要。
	\include{tex/abstract}

	% 自动生成目录。
	\tableofcontents

	% 以下为正文。
	\mainmatter

	\input{tex/intro}
	\input{tex/related}
	\input{tex/algo}
	\input{tex/application}
	\input{tex/eval}
	\input{tex/anal}
	\input{tex/visual}
	% \input{tex/6.other}
	\input{tex/future}
	\input{tex/conc}


	% % 绪言。
	% \include{chap/introduction}
	% % 各章节。
	% \include{chap/chap1}
	% \include{chap/chap2}
	% \include{chap/chap3}
	% % 结论。
	% \include{chap/conclusion}

	% 正文中的附录部分。
	\appendix
	% 排版参考文献列表。
	\printbibliography[
		% 使“参考文献”出现在目录中；如果同时要使参考文献列表参与章节编号，
		% 可将“bibintoc”改为“bibnumbered”。
		heading = bibintoc
		% 单独设定排序方案。此设定会局部覆盖之前的全局设置。
		% 注：只有同时使用 2.x 或之后版本的 biblatex 和相应兼容版本的 biber，
		% 才能对每个 \printbibliography 命令采用不同的排序方案，
		% 否则只能在导入 biblatex 宏包时就（全局）指定排序方案。
		% sorting = ecnty
	]
	% 各附录。
	% \include{chap/encl1}
	% \include{chap/encl2}
	
	% \bibliographystyle{abbrv}
	% \bibliography{tex/pkuthss}

	% 以下为正文之后的部分。
	\backmatter

	% 致谢。
	\include{tex/acknowledge}
	% 原创性声明和使用授权说明。
	\include{chap/originauth}

\end{document}

